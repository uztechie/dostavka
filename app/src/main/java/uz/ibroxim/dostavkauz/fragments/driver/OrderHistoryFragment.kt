...
@AndroidEntryPoint
class OrderHistoryFragment:Fragment(R.layout.fragment_new_orders) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        newOrderAdapter = NewOrderAdapter(StatusList.Accepted.id, requireContext(), object : NewOrderAdapter.NewOrderAdapterCallBack {
            override fun onItemClick(order: Order) {
                ...
                findNavController().navigate(OrderHistoryFragmentDirections.actionOrderHistoryFragmentToAcceptedOrderDetailsFragment(order))
            }
        ...
        viewModel.orderHistoryResponse.observe(viewLifecycleOwner){ response->
            when(response){
                ...
                is Resource.Success->{
                    ...
                    response.data?.let {
                        if (it.status == 200){
                            val list = it.data?.filterNot { order->
                                order.status_id == 1
                            }
    ...
    private fun initToolbar(){
        ...
        toolbar_title.text = getString(R.string.buyurtmalar_tarixi)
